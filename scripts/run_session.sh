...
/home/home/.opencode/bin/opencode run \
  ...
  "你是这台电脑的管理 Agent（身份与权限见 workspace/agent.md）。本次是延续上次会话的新会话，没有对话历史。请先阅读 workspace/handoff.md 与 workspace/memories/ 下的记忆，恢复当前状态和任务，然后继续或完成维护工作。只做不依赖 sudo 的操作；需要 sudo 的事项写入 handoff.md 的'需要 sudo 的事项'。完成后更新 handoff.md：更新时间、状态、当前状态、下次任务、下次运行（无需继续则清空），并简要汇报结果。" >> "$LOG" 2>&1
rc=$?
